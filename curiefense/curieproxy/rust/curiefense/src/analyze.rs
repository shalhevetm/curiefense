...
use crate::grasshopper::{challenge_phase01, challenge_phase02, GHResponse, Grasshopper};
...
pub fn analyze_init<GH: Grasshopper>(logs: &mut Logs, mgh: Option<&GH>, p0: APhase0) -> InitResult {
    ...
    let gh_response = p0.gh_response;
    ...
    let decision = if let SimpleDecision::Action(action, reason) = globalfilter_dec {
        ...
        let decision = action.to_decision(&gh_response, mgh, &reqinfo, &mut tags, reason);
...
pub fn analyze_finish<GH: Grasshopper>(
    ...
    let gh_response = info.gh_response;
    ...
    if let SimpleDecision::Action(action, curbrs) = limit_check {
        let limit_decision = action.to_decision(&gh_response, mgh, &reqinfo, &mut tags, curbrs);
    ...
    let acl_decision = acl_result.decision(gh_response.precision_level.is_human());
    ...
    if let Some(decision) = acl_decision {
        ...
        let acl_block = |tags: &mut Tags| {
            secpol
                .acl_profile
                .action
                .to_decision(&gh_response, mgh, &reqinfo, tags, Vec::new())
        };

        // Send challenge, even if the acl is inactive in sec_pol.
        if decision.challenge {
            let decision = if let Some(gh) = mgh {
                challenge_phase01(gh, Vec::new(), gh_response)
            } else {
